...
- **v2.7.53 (Manual Assessment Catch-up Push, 2026-06-17):** Admins can now push a catch-up assessment directly to a selected trainee from Test Engine without requiring a Schedule Studio timeline item. The shared `manual_assessment_assignments` document is treated as a guarded server-authoritative app document, trainee notifications surface active manual assignments, and both legacy Test Engine assessments and Assessment Studio generators mark the manual assignment started/submitted as the trainee progresses. Assessment Studio manual pushes generate one sealed trainee snapshot and keep that manual attempt keyed separately from normal scheduled generator assignments. Regression verification: full Jest suite.
