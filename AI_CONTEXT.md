...
| `sso_login_config` | Object | Blob (`app_documents`) | Optional SSO login configuration. Base implementation supports Supabase OAuth provider `azure`, custom callback `first-line-training://auth/callback`, optional `allowedDomains`, and maps authenticated email back to an existing app user before app access is granted. |
...
- **SSO Base Prep (2026-06-01):** The app has a first-pass optional Microsoft/Azure SSO login path. Electron registers `first-line-training://auth/callback`, starts Supabase OAuth in the system browser, receives the callback, and maps the authenticated email/UPN to an existing app user (`user`, `email`, or `traineeData.email`). SSO does not create app users or bypass current roles, revoked-user checks, blocked status, boot-mode gates, or client binding. Enable only after Supabase Auth provider + redirect URLs are configured and `sso_login_config.enabled` is set true.
