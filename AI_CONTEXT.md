...
- **v2.7.82 (Live Assessment Start Hotfix, 2026-06-22):** `js/assessment_trainee.js` now attaches Device Session context only when a matching physical-router session is actually claimed. Standard Test Engine assessments keep a `null` launch context and no longer throw `Cannot set properties of null (setting 'deviceSession')` when trainees click Start Assessment.
...
| `course_progress_request_config` | Object | Blob (`app_documents`) | Admin Tools > System Config recipients, SMTP settings, editable trainee request message/button text, editable email body template, and editable acknowledgement message for Schedule Studio move-on requests. |
...
| `sso_login_config` | Object | Blob (`app_documents`) | Optional Microsoft SSO login configuration. Uses Supabase OAuth provider `azure`, an in-app Electron auth window with callback `first-line-training://auth/callback`, optional `allowedDomains`, and strict existing-user mapping by explicit SSO provider ID/email or exact stored profile email before app access is granted. |
...
- **v2.7.81 (Assessment Studio Hard Save Guard, 2026-06-22):** Assessment Studio module saves, host loader direct fallback saves, and global blob sync now protect recovered/non-empty server Question Bucket/generator/grouping/tag data from stale clients whose local cache only contains submissions. If local authoring is empty while server authoring exists, module/global saves preserve server authoring and merge submissions; direct host fallback refuses the upload. Added `ops/assessment_studio_authoring_guard_20260622.sql` for a Supabase trigger that preserves existing authoring arrays if a stale client attempts an empty-authoring update. This is the push build after the snapshot-based recovery of Assessment Studio authoring.

- **v2.7.80 (Assessment Studio Authoring Guard, 2026-06-22):** Assessment Studio, host loader, global sync, and Schedule Studio merge paths now preserve local Question Bucket/generator/grouping/tag data when a newer partial server document only carries submissions. This release is the post-incident safety build to push before further Assessment Studio editing or row backfill work; if the live server document was already overwritten, recover authoring from server backup or submitted `testSnapshot.questions` first, then keep the existing submissions intact. Violation evidence signed URL creation also retries brief Storage/database timeouts.

- **v2.7.79 (Assessment/Violation Exact Rows + Device Sessions, 2026-06-22):** Assessment Studio submissions and violation reports now have exact-row tables for live-critical submit, grading, lock, and review paths. Super Admin Data & Logs includes a guided copy-only backfill that previews counts, writes missing or older same-state rows, and skips existing newer/completed/reviewed row data to avoid downgrading review records. Assessment Studio, host loader, global sync, and Schedule Studio merge logic preserves local Question Bucket/generator/grouping/tag data when a newer partial server document only carries submissions. Admin Tools also includes Device Sessions for selected Test Engine or Assessment Studio assessments; the runtime claims one available configured device session, injects client code/MAC/PPPoE context into the assessment, and marks the session Requires Attention after submit until an admin releases it. Background Network Diagnostics remains local-only unless `enable_network_diagnostics_cloud` is explicitly set, orphan cleanup uses throttled probes for high-volume tables, and violation evidence signed URL creation retries brief Storage/database timeouts. Device Sessions require `ops/assessment_device_sessions_20260622.sql` before use.
- **v2.7.78 (Realtime Outage Backoff + Evidence Recovery, 2026-06-19):** Realtime fallback polling now reads `sessions`, `live_sessions`, `vetting_sessions`, and `vetting_sessions_v2` sequentially, pauses early after repeated 502/503/schema-cache outage errors, and lets the existing fallback rate backoff stretch retries instead of hitting every table in parallel. Violation evidence review now tolerates missing or unsigned Storage objects per screenshot and renders an unavailable slot rather than failing the whole evidence modal.

- **v2.7.77 (Violation Evidence API Burst Relief, 2026-06-19):** Violation evidence review now creates Storage signed URLs sequentially instead of with `Promise.all`, and approved evidence cleanup deletes Storage objects plus updates `violation_evidence` metadata one item at a time with a short delay. This reduces Storage/PostgREST bursts when admins rapidly approve evidence-backed violation reports while the self-hosted Supabase stack is under pressure.

- **v2.7.76 (Cached Startup + Safer Grading Lock Claim, 2026-06-18):** App boot now races the initial `loadFromServer(true)` against a short startup window so cached UI/login is not held hostage by slow Supabase row-table reads. High-priority tab refreshes call `runBackgroundCloudSync(...)` instead of blocking the route loader on PostgREST. Assessment Studio `selectSubmission(...)` now renders the grader workspace before claiming the grading lock, so a failed open cannot leave a stale "You are grading" badge. `link_requests` also moved to the smaller row-sync limit/cooldown group.

- **v2.7.75 (Row Sync Pressure Relief, 2026-06-18):** `loadFromServer()` now runs normal row-table pulls through a controlled two-worker queue instead of launching every row table request at once. Large row tables use smaller limits, statement-timeout failures set a five-minute per-key cooldown, and `monitor_state` uses the same timeout backoff. Normal admin navigation no longer runs the broad `records/submissions select id limit 10000` reconciliation scan unless `enable_full_row_reconcile` is explicitly enabled in local storage.

- **v2.7.74 (Super Admin DevTools Bridge, 2026-06-18):** The Super Admin Console DevTools action now uses `window.electronAPI.ipcRenderer.send('open-devtools')` before falling back to direct `require('electron')`, and `electron-main.js` no longer blocks the IPC handler in packaged builds. DevTools open detached from the main app window.

- **v2.7.73 (App Document Pull Timeout Isolation, 2026-06-18):** `loadFromServer()` now fetches stale `app_documents` keys individually (`APP_DOCUMENT_FETCH_BATCH_SIZE = 1`) and treats a per-key timeout/500 as a skipped document instead of failing the whole server pull. This specifically protects startup/dashboard sync from a single slow `rosters`, `schedules`, `liveSchedules`, or `tests` content fetch leaving the app at an old "Last synced" timestamp.

- **v2.7.72 (Disabled Error Reports Live Sync, 2026-06-18):** `error_reports` is now treated as a disabled legacy diagnostics dataset in `js/data.js`. The runtime clears local legacy error-report cache at startup, `reportSystemError()` returns disabled without collecting rows, `saveToServer()` filters the key out of normal queues, and app-document metadata pulls skip any old `error_reports` blob. The server table may still exist for historical maintenance, but it is no longer part of normal app sync.

- **v2.7.71 (Error Report Sync Timeout Relief, 2026-06-18):** Runtime `reportSystemError()` now keeps `error_reports` local-only instead of immediately queueing a Supabase upload; this prevents server timeouts from recursively creating more error-report uploads. Normal/non-forced sync defers `error_reports`, broad local-cache republish excludes it, and forced explicit maintenance uploads cap error report rows/chunks to reduce PostgREST statement timeout risk. Manual sync retry now pings `public.app_health` first.

- **v2.7.70 (Trainee Day-End Logout + Active Session Cleanup, 2026-06-18):** Trainee sessions now schedule a once-per-day 17:30 sign-out in `js/auth.js`; the guard records the date locally before logout so trainees can sign back in afterward without a logout loop. Logout/day-end paths mark the current `sessions` row inactive by writing an older `lastSeen` and signed-out activity text, while `js/data.js` drops signed-out or stale session realtime rows from `ACTIVE_USERS_CACHE`. Network Diagnostics now probes `public.app_health` first and falls back to `app_documents` only if the tiny health table is unavailable.

- **v2.7.69 (Assessment Studio Local Submit + Scoring Fix, 2026-06-18):** Multiple-answer auto marking now only deducts when the trainee over-selects beyond the number of correct answers; wrong choices inside the allowed answer count simply miss credit for the correct choice not selected. Feedback Sessions preserves search/filter/date controls after feedback status saves. Trainee Assessment Studio submit now seals the completed snapshot locally, blocks reopening, returns to My Assessments, and shows an explicit `Upload Assessment` action instead of immediately pushing to Supabase; Supabase presence checks and re-upload recovery remain available for missing server copies. Current-version admin heartbeat loops now use a calmer 15-second UI cadence and write DB heartbeat rows no more than every 30 seconds to reduce session-table pressure. Regression verification: focused Assessment Studio grading/runtime tests and full Jest suite.

- **v2.7.68 (Supabase Hotspot Relief, 2026-06-18):** Assessment Studio grading lock heartbeats now run every two minutes with a five-minute lease instead of forcing a full `assessment_studio_data` document save every 45 seconds. Added `ops/supabase_server_hotspot_diagnostics_20260618.sql` for direct Docker/psql hotspot reporting, autovacuum tuning, index checks, analyze, and normal `VACUUM ANALYZE` on `app_documents`. Updated the Supabase server compose copy with conservative Postgres memory/checkpoint tuning for the 32 GB RAM host. The remaining structural bottleneck is that Assessment Studio still rewrites one shared JSON document for submissions, question saves, and grading; the long-term fix is to move submissions/locks into row-level tables.

- **v2.7.67 (Assessment Studio Trainee Upload Retry Queue, 2026-06-18):** Failed trainee Assessment Studio submissions now enter a local background upload queue instead of relying on repeated manual Re-upload clicks. Manual retry queues one immediate background attempt, queued cards display `Upload Queued` / `Retry Scheduled`, and failed attempts back off up to two minutes while preserving the local submitted snapshot. Regression verification: focused Assessment Studio trainee runtime tests and full Jest suite.

- **v2.7.66 (Assessment Studio Per-Question Marker Comments, 2026-06-18):** Assessment Studio grading now renders a marker comment textarea on every question and persists those comments as `questionComments` alongside `questionScores` when the submission is completed. Normalization preserves `questionComments` so comments survive reload/sync. Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.65 (Assessment Studio Grading Lock Heartbeat, 2026-06-18):** Assessment Studio grading locks now use a 3-minute heartbeat lease instead of a fixed 30-minute reservation. Confirmed open graders heartbeat every 45 seconds to keep the lock alive, while locks whose heartbeat stops become available quickly. Current admins can release their own older-session locks when returning to the queue or refreshing. Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.64 (Assessment Studio Grading Open Proof, 2026-06-18):** Assessment Studio now verifies that the selected grader workspace mounted after a grading lock claim. If the workspace does not open, it releases the lock immediately, clears the selection, and returns the admin to the queue with an error instead of leaving the row marked as "you are grading" or "`name` is grading." Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.63 (Assessment Studio Grading Lock Recovery, 2026-06-18):** Assessment Studio grading locks now treat an active lock with the current admin marker name as recoverable even if it came from an older app session, preventing failed opens/reloads from leaving the same admin blocked by their own "`name` is grading" badge. Different-admin locks remain protected. Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.62 (Sync Freeze Guard, 2026-06-18):** The global Supabase sync busy overlay is now informational rather than click-blocking, and it auto-dismisses after a long-running sync with a warning toast so slow server calls cannot leave the visible app frozen. Regression verification: focused syntax/sync checks and full Jest suite.

- **v2.7.61 (Realtime Listener Trim, 2026-06-18):** Trainee realtime no longer listens to heavy `monitor_history` and `nps_responses` row tables, and Super Admin Data Studio no longer opens a schema-wide realtime subscription. It now binds only the explicit table list it can display and excludes low-priority diagnostic/history tables from live fan-out. Regression verification: focused sync/Assessment Studio tests and full Jest suite.

- **v2.7.60 (Assessment Studio Immediate Bucket Refresh, 2026-06-18):** Newly saved Question Bucket questions now repaint the visible bucket rows and assessment point totals immediately after the local save, while the modal stays ready for the next question and Supabase verification/re-upload continues in the background. Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.59 (Assessment Studio Question Recovery, 2026-06-18):** Question Bucket saves now verify that the saved question exists in the remote Supabase Studio document after upload confirmation. Bucket rows show an Upload Failed badge and Re-upload Question action for local questions that are missing, stale, or failed on Supabase; retry merges just the local question plus grouping/tag metadata back into the shared Studio document. Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.58 (Assessment Studio Question Save Hotfix, 2026-06-18):** Question Bucket saves now commit locally and update/reset the admin UI immediately before waiting for Supabase confirmation, preventing repeated Save clicks when the server is slow. New grouping/tag selections made from the question modal are folded into the same Studio save instead of causing separate blocking cloud saves first. Regression verification: focused Assessment Studio grading tests and full Jest suite.

- **v2.7.57 (Supabase Realtime Load Relief, 2026-06-18):** Admin realtime subscriptions now keep operational updates active while excluding heavy diagnostic/history tables from realtime fan-out, and the general server pull skips the largest diagnostic/history row tables so login, Assessment Studio, violations, and submissions are not blocked by broad background scans. Remote session command polling remains as a fallback but is throttled while realtime is healthy, and embedded Assessment Studio saves avoid duplicate host cloud writes after Supabase confirmation. Added `ops/supabase_realtime_load_relief_20260618.sql` for the matching Supabase publication/index/analyze maintenance. Regression verification: focused sync tests and full Jest suite.

- **v2.7.56 (Assessment Studio Trainee Matrix + Feedback Status, 2026-06-17):** Assessment Studio Feedback Sessions now normalize and persist `none`, `requested`, and `received` statuses onto the shared Studio submission and host cache so admin changes survive refresh and reflect outside the embedded module. The trainee runtime matrix layout now adapts column widths for wider grids, keeps row labels sticky during horizontal scroll, and adds bottom spacing so the sticky Save Draft / Submit action bar does not cover answer rows. The trainee progress sidebar now uses the same completeness rules as submit validation, and draft save can clear visible multi-select/matching/matrix answers instead of preserving stale selections. Regression verification: focused Assessment Studio grading, sync, and trainee runtime tests.

- **v2.7.55 (Studio Draft + Schedule Window Guard, 2026-06-17):** Assessment Studio trainee draft save and final submit now flush the visible runtime controls before saving, preventing recent radio/checkbox/select/text changes from disappearing when the save triggers a refresh. Studio timeline launches now enforce the saved Schedule Studio assessment window on the trainee runtime side as well as in Schedule Studio, including due date, open/close time, ignore-time, and trainee exception settings; expired or time-closed Studio timeline items cannot create or reopen a trainee snapshot. Regression verification: full Jest suite with focused runtime coverage for draft preservation and expired/time-closed Studio schedules.

- **v2.7.54 (Assessment Studio Catch-up Placement, 2026-06-17):** Moves the manual catch-up assignment admin workflow out of the legacy Test Engine overview and into Assessment Studio. The Assessment Studio sidebar submenu now includes Catch-up Push, the Studio header exposes the same action, and the panel filters to Assessment Studio generators with assessment selection before trainee selection. Regression verification: focused catch-up admin tests plus trainee submit/runtime coverage.

- **v2.7.53 (Manual Assessment Catch-up Push, 2026-06-17):** Adds the underlying manual catch-up assignment workflow so admins can push a catch-up assessment directly to a selected trainee without requiring a Schedule Studio timeline item. The shared `manual_assessment_assignments` document is treated as a guarded server-authoritative app document, trainee notifications surface active manual assignments, and both legacy Test Engine assessments and Assessment Studio generators mark the manual assignment started/submitted as the trainee progresses. Assessment Studio manual pushes generate one sealed trainee snapshot and keep that manual attempt keyed separately from normal scheduled generator assignments. Regression verification: full Jest suite.
...
- **v2.7.23 (Assessment Studio Document Sync Authority, 2026-06-10):** Assessment Studio uses the blob key `assessment_studio_data` rather than legacy row-level `submissions`, so stale local caches could resurrect deleted Test Generator Details or deleted/pending/completed Assessment Studio submissions. The sync engine now allows trainees to pull `assessment_studio_data`, treats server-side Assessment Studio document pulls as authoritative for admin structures, preserves only newer current-trainee local drafts/submissions, and refreshes My Assessments / active trainee runtime on Assessment Studio document realtime changes. Schedule Studio now chooses the Assessment Studio cache by `updatedAt` timestamp instead of item count and preserves generator `pointLeeway` when linking timeline items. Regression coverage: `tests/sync.test.js` server-delete and newer-current-trainee draft cases.

- **v2.7.21 (Assessment Studio Realtime Synchronization Hardening, 2026-06-10):** Critical realtime sync reliability fix for assessment grading and deletion workflows. When an admin completes marking a test, the submission and record updates now **always** trigger UI refresh across all active admin instances (marking queue, test records, dashboard) regardless of whether the target tab is active. This ensures all admins see completed tests removed from marking queues instantly. Additionally, trainees now see completed assessments with updated marks immediately via a new realtime listener in `assessment_trainee.js` that auto-refreshes My Assessments when submission/record changes arrive. **Submission and record deletions now propagate properly to all users** by explicitly emitting `buildzone:data-changed` events and calling `saveToServer()` after successful `hardDelete()` in both `admin_grading.js` and `admin_history.js`, ensuring other admins' queues and trainees' assessment lists update instantly. Changes: `js/data.js` processIncomingDataQueue() tracks submission/record changes and explicitly calls role-based refresh functions; `js/assessment_trainee.js` listens for `buildzone:data-changed` events; `js/admin_grading.js` and `js/admin_history.js` emit data-changed events and sync deletions. This fixes: "when submitted it doesnt synch to all admins..." and "when i delete a submission it doesnt delete to all users." Full Electron main-channel release.

- **v2.7.11 (Completed Assessment Regrade Repair Release, 2026-06-01):** Full Electron main-channel release replacing the reused `2.7.10` artifact. Includes the completed-assessment edit hotfix plus the controlled score-map repair path for odd completed tests such as Fibre Slow Speed where the saved per-question `scores` map is missing. Admin edit mode can now repair those records after reviewing every mark; view mode remains read-only. Trainee-facing release notes remain suppressed.
- **Retrain Migration Integrity (2026-06-04):** Admin Tools > Manage Users move/retrain flow must archive the outgoing attempt before clearing live rows, using `insight_progress_config`/ProgressCatalog as the Agent Progress Builder source of truth. Retrain archives now include active `liveSessions` along with records, submissions, attendance, saved reports, reviews, exemptions, live bookings, link requests, monitor history, TL task submissions, notes, `progressConfigSnapshot`, and `officialProgress`. Cleanup must match trainee identity across `trainee`, `user`, and `user_id` fields, delete live-session rows by `sessionId`, remove the trainee from all previous roster groups case-insensitively, then add them to the selected target group for a clean current attempt. Regression coverage: `tests/retrain_migration.test.js`.
- **SSO Base Prep (2026-06-01):** The app has an optional Microsoft/Azure SSO login path. Electron opens Supabase OAuth in a compact in-app auth window using the same persistent `persist:study_session` partition as the Study Browser, so Microsoft/SharePoint study material can reuse the signed-in session where Microsoft allows it. The app captures `first-line-training://auth/callback` inside the auth window and maps the authenticated Microsoft identity to an existing app user by explicit `ssoProviderId`/`ssoObjectId`/`ssoLoginId`, explicit `ssoEmail`, or exact stored email (`email`, `traineeData.email`, `contactEmail`, or username only when it is a full email address). Manage Users > Advanced Edit exposes Microsoft SSO Email and Microsoft SSO ID fields so admins can pin the mapping before rollout. Super Admin Console > Configuration exposes Microsoft SSO Login controls for enable/disable, provider, allowed domains, and exact-email fallback, saving to synced `sso_login_config`. Do not reintroduce loose email-prefix/name matching. SSO does not create app users or bypass current roles, revoked-user checks, blocked status, boot-mode gates, or client binding. Enable only after Supabase Auth provider + redirect URLs are configured and `sso_login_config.enabled` is set true.
- **v2.7.10 (Completed Assessment Regrade Hotfix, 2026-06-01):** Full Electron main-channel hotfix. Completed Assessments > Edit now keeps the `Save Changes` action visible and wired for admins when adjusting already graded digital submissions. This fixes an edit-mode button regression without changing trainee-facing release notes.
...
- **v2.7.0 (Migration + Navigation + Stability Cleanup, 2026-05-21):** Prepared the main-channel rollout after the morning cleanup series. Retrain migration now treats Agent Progress Builder as the official archive checklist, preserves valid N/A marks, blocks cleanup when archive persistence fails, and makes Insight Studio's migrate action show the host group selector reliably. Trainee login and navigation now avoid several blocking refresh paths, while the admin sidebar uses a cleaner Navigation Map with drag-and-drop ordering and inline quick submenus for major module destinations. Live Assessment Arena trainee/admin layouts now compact only the arena surface when UI zoom is high and keep question, answer, chat/message, scoring, and action controls visible inside the available viewport. The cleanup sweep removed direct `JSON.parse(localStorage.getItem(...))` hot paths across the main shell, auth, calendar, diagnostics, assessment/admin workflows, Insight, Vetting, Content Studio, Team Hub, OPL Hub, Q&A Hub, reporting/search/dashboard, and related modules so malformed local cache values are far less likely to crash boot, login, migration, or navigation. This ships as stable main-channel version `2.7.0`.
...
- Vetting Grading: New Vetting Arena submissions now enter pending admin review, but historical completed Vetting rows are no longer reopened purely because they lack newer marking audit metadata.
- Grading Queue: Linked pending submissions with permanent records are repaired back to completed, preventing previously reviewed Vetting attempts from flooding pending review.
- Insight Knowledge Gaps: Individual view now shows below-full-mark questions for the selected trainee with full question text, trainee answer, and awarded/max points.
- Insight Knowledge Gaps: Group view now aggregates failed questions by group and assessment, with an assessment filter for cleaner training-gap analysis.
- HR Evidence: Incidents can now be linked to multiple evaluation triggers, and captured evidence supports edit, delete, and trigger filtering.
- Insight Polish: Assessment/Test Breakdown graphs now use readable assessment names on the axis instead of numeric-only labels.
- Reliability: Release cleanup includes the accumulated Vetting Arena, Insight, Schedule Studio, dashboard, auth, sync, navigation, and embedded module hardening from this update cycle.
- Release: Version bump to `2.7.3` for stable main-channel rollout.

## v2.7.2 - 2026-05-26

- Vetting Arena: Admin group selection now displays readable month/year group names, member counts, and trainee previews.
- Vetting Arena: Test selection is grouped into 1st Vetting, Final Vetting, and Other Vetting sections with selected-group completion tracking to avoid repeating the wrong test.
- Vetting Reliability: Starting a session seeds the selected group as waiting, nudges targeted trainees, and preserves per-trainee status updates for monitoring.
- Trainee Flow: Security pre-flight scans use direct preload security APIs and no longer recreate the join screen after successful checks, keeping the Enter Arena button stable.
- Vetting Reliability: Ending a session now sends targeted trainees an explicit release command and ignores stale inactive rows while server deletes finish.
- Polish: Vetting admin and trainee views now use a focused command-center and secure exam profile.
- Cleanup: Removed stale standalone Vetting Rework route references in favor of the official Vetting Arena route.
- Release: Version bump to `2.7.2` for stable main-channel rollout.
...
- Feature: Course move-on requests now send through synced SMTP settings before falling back to local mail options.
- Feature: Successful course move-on requests now create `admin_notifications` entries for admin/super-admin notification bells.
- Improvement: System Config now includes SMTP host, port, SSL/TLS, username, password, and from-address fields for the request email sender.

## v2.6.86 - 2026-05-13

- Feature: Schedule Studio timeline items can show an optional trainee move-on request button.
- Feature: Admin Tools > System Config now saves request recipients and the trainee acknowledgement message through `course_progress_request_config`.
- Improvement: Timeline item edit now supports assigned-group trainee availability exceptions that bypass the normal date gate for selected users.

## v2.6.85 - 2026-05-13

- Feature: Compare Viewer Two Graphs mode now has First and Second group selectors for direct group-vs-group Assessment/Test Breakdown comparison.
- Improvement: Split graph panels render the people inside each selected group instead of dividing the current result list into two arbitrary halves.
...
- Feature: Compare Viewer Assessment/Test Breakdown graph now has Single Graph and Two Graphs display modes.
- Improvement: Two Graphs mode splits the current selected people/groups into two separate chart panels for cleaner reading.
- Fix: Insight sidebar icon now uses the supported `fa-chart-line` icon so the view has a visible navigation symbol.
...
- Improvement: Insight now displays an immediate loading screen with the message "Fetching and building records" while its data workspace is prepared.
- Performance: Host webview creation is delayed briefly so the loading screen can paint before heavier snapshot hydration starts.
- Improvement: The Insight module uses the same loading message during its own data preparation state.

## v2.6.80 - 2026-05-13

- Improvement: Compare Viewer Assessment/Test Breakdown graph now scales within the visible card instead of forcing horizontal page overflow.
- Improvement: Graph end labels were replaced with a compact summary/legend area to reduce clutter when many people or groups are selected.
- Improvement: Compare Viewer cards, filter picker, graph headers, axis key, and responsive mobile behavior were visually tightened.
