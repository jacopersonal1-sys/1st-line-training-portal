...
## Recent Major Updates (AI Context)
 - **v2.6.22**: **User Control Explorer + Study Browser Hit-Test Hotfix**: Expanded Super Admin Data Studio `User Control` with an `Agent Data Explorer` folder model that shows detailed archive buckets and supports specific row moves both directions (live↔archive) with backup snapshots (`user_control_move_backups`) and guarded rollback behavior. Also hardened Study Browser/webview layering and hidden-tab hit-testing to fix inconsistent unclickable zones in embedded program pages (e.g., Q-Contact).
 - **v2.6.21**: **Content Creator Media + Engagement Expansion**: Upgraded the isolated `content-studio` runtime into a full Content Creator delivery flow with optional per-subject video/document toggles, upload-source selection (HTTP or direct Supabase storage upload for video/PDF), admin-only Engagement submenu analytics (per-user + per-subject breakdown), and timestamp-linked video notes/questions captured per user directly from the player.
 - **v2.6.20**: **Content Studio + User Control Release**: Added a new isolated `content-studio` module (View + Builder) with schedule-linked headers/subjects, video/document actions, and per-user video engagement telemetry (plays/watch-time/skips). Expanded Super Admin Data Studio with a dedicated User Control workspace for revoke/binding management, archive-attempt editing, and one-click archive/reset lifecycle cleanup across live rows.
 ...
 - **v2.6.13**: **Recovery Safety + Vetting Enforcement Reliability**: Fixed Vetting 2.0 trainee security relax/override application during active sessions by hardening user identity resolution across runtimes, made explicit strict shared-key saves flush immediately to reduce rollback windows for users/tests/schedules, and changed JSON restore sync behavior to upload only imported keys (instead of force-pushing all keys) to prevent stale unrelated data from being republished.
